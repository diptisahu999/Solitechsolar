...
class InsPartnerLedger(models.TransientModel):
    ...
    _description = "Ins Partner Ledger"
    ...
    all_partner = fields.Boolean(
        string='All Partners', default=False
    )
    ...
    def prepare_from(self):
        sql_from = '''
            ...
            JOIN account_account a ON (l.account_id=a.id AND COALESCE(a.not_view_in_report, FALSE) != TRUE)
    ...
    def prepare_main_lines(self):
        ...
        partner_ids = []
        if self.all_partner or self.partner_ids:
    ...
    def prepare_detailed_lines(self, move_line_ids=[], partner_id=False):
        ...
        sql = ('''
                SELECT
                    ...
                    CASE
                        WHEN l.currency_id <> l.company_currency_id THEN sum(l.amount_currency)
                        ELSE 0
                    END AS amount_currency,
               ...
               ''' AND l.partner_id = %s 
               GROUP BY
                   l.date,  m.id, l.matching_number,
                    l.date_maturity, j.code, a.name, l.currency_id, a.code, m.name,l.company_currency_id 
               ORDER BY l.date
           ''' % partner_id)
        cr.execute(sql)
    ...
    def update_values_from_component(self, vals):
        ...
        if vals.get('all_partners',False):
            update_dict.update({'all_partner':True})
        else:
            update_dict.update({'all_partner':False})
