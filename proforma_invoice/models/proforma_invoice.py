...
class ProformaInvoice(models.Model):
    ...
    sr_number = fields.Integer(string='Revision No.', copy=False, default=1)
    ...
    # def write(self, vals):
    #     """Override write method to increment sr_number when any field actually changes.

    #     Improvements:
    #     - Compare new values against existing values to avoid incrementing on prints
    #       or other system operations.
    #     - Handle common command formats for m2m/o2many (6, 4) to detect real changes.
    #     - Increment per-record so each PI gets its own sequential revision.
    #     - Use a context flag `skip_revision_increment` to avoid nested increments.
    #     """
    #     # Fields to exclude from triggering sr_number increment
    #     excluded_fields = {
    #         'sr_number',        # Don't increment if only sr_number changes
    #         '__last_update',
    ...
    #                 continue
    #             try:
    #                 same = _is_same_value(pi, key, new_val)
    #             except Exception:
    #                 same = False
    #             if not same and pi.state != 'cancel':
    ...
    #             if key in excluded_fields:
    #                 continue
    #             # special handling: avoid dumping full line_ids dicts in the log
    #             if key == 'line_ids':
    #                 # if it's initial creation, skip logging line_ids noise
    ...
    #                     summary_text = ', '.join(summary) or 'invoice lines changed'
    #                     per_changes.append(('Invoice Lines', '', summary_text))
    #                     # continue to next field
    #                     continue
    #                 except Exception:
    #                     # fallback: skip heavy line_ids logging
    #                     continue
    #             try:
    #                 same = _is_same_value(pi, key, new_val)
    #             except Exception:
    ...
    #         if per_changes:
    #             rev = (pi.sr_number or 0) + 1
    #             write_vals = dict(vals)
    #             write_vals['sr_number'] = rev
    #             # perform the actual write but avoid nested revision increments
    #             pi.with_context(skip_revision_increment=True).write(write_vals)

    #             # Post a message with the revision and change log
    #             body_lines = [f'<b>Revision:</b> {rev:04d}', '<b>Changes:</b>']
    #             for lbl, o, n in per_changes:
    #                 body_lines.append(f'{lbl}: {o} → {n}')
    #             body = '<br/>'.join(body_lines)
    #             try:
    #                 pi.message_post(body=body, subject=f'PI updated (Revision {rev:04d})')
    #             except Exception:
