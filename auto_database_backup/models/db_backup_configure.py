...
class DbBackupConfigure(models.Model):
    ...
    def _schedule_auto_backup(self):
        ...
        for rec in records:
            ...
            static_dir = os.path.join(module_dir, '..', 'static', 'hz_icon')
            if rec.backup_destination == 'local':
                print("==========rec.backup_destination=========",rec.backup_destination)
                try:
                    print("------------try----------",os.path.isdir(rec.backup_path))
                    print("------------try----------",(rec.backup_path))
            ...
            # Google Drive backup
                ...
                print("-------rec.gdrive_token_validity-----",rec.gdrive_token_validity)
                print("-------rec.gdrive_token_validity-----",fields.Datetime.now())
                try:
